...
class DocumentResourceTest {
    ...
    void testSetupInTransitOperation() {
        ...
        inTransitOperationId = given()
                ...
                .contentType(ContentType.JSON)
                .body("""
                        {"clientId": 1, "operationType": "IMPORT", "transportMode": "AIR", "operationCategory": "CATEGORY_1", "blNumber": "BL-RR-TEST", "estimatedArrival": "2025-12-01T10:00:00", "blAvailability": "ORIGINAL", "incoterm": "FOB"}
                        """)
                .when().post("/api/operations")
                .then().statusCode(201)
                .extract().jsonPath().getLong("id");
