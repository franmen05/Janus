...
class PermissionHardeningTest {
    ...
    void testSetupOperations() {
        ...
        otherClientOperationId = given()
                ...
                .contentType(ContentType.JSON)
                .body("""
                        {"clientId": 2, "transportMode": "MARITIME", "operationCategory": "CATEGORY_3", "containerNumber": "CONT-002", "blNumber": "BL-TEST-002", "estimatedArrival": "2025-12-01T10:00:00", "blAvailability": "ORIGINAL"}
                        """)
                .when().post("/api/operations")
                .then().statusCode(201)
                .extract().jsonPath().getLong("id");
